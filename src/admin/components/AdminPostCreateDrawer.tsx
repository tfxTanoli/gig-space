...
interface Props {
  onClose: () => void;
  onSuccess: (created: AdminService) => void;
}
...
export default function AdminPostCreateDrawer({ onClose, onSuccess }: Props) {
  ...
  const handleSave = async () => {
    ...
    if (parsedMax !== null && parsedMax < parsedMin) { setError('Price Max must be Ã¢â€°Â¥ Price Min.'); return; }
    ...
    try {
      ...
      const postRef = push(dbRef(database, 'services'));
      const newId = postRef.key!;

      for (const file of files) {
        const path = `services/${newId}/${Date.now()}_${file.name}`;
      ...
      <div className="fixed right-0 top-0 h-screen w-full max-w-xl bg-surface border-l border-slate-700 z-50 flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Create a service post from scratch</p>
          </div>
              ...
              <div className="flex flex-wrap gap-2">
                {extraLocations.map((loc) => (
                  <span key={loc} className="flex items-center gap-1.5 text-xs bg-slate-800 text-slate-300 px-2.5 py-1 rounded-full border border-slate-700">
                    {loc}
                    <button onClick={() => setExtraLocations((p) => p.filter((l) => l !== loc))} className="hover:text-red-400 transition-colors"><X className="w-3 h-3" /></button>
                  </span>
                ))}
              </div>
            )}
              ...
              <div className="flex flex-wrap gap-2">
                {languages.map((lang) => (
                  <span key={lang} className="flex items-center gap-1.5 text-xs bg-slate-800 text-slate-300 px-2.5 py-1 rounded-full border border-slate-700">
                    {lang}
                    <button onClick={() => setLanguages((p) => p.filter((l) => l !== lang))} className="hover:text-red-400 transition-colors"><X className="w-3 h-3" /></button>
                  </span>
                ))}
              </div>
            )}
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-t border-slate-800 bg-surface flex-shrink-0">
          ...
          <button onClick={handleSave} disabled={saving} className="flex-1 py-2 rounded-lg bg-primary hover:bg-blue-400 text-white text-sm font-semibold transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
